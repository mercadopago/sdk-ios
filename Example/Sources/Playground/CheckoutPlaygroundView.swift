...
struct CheckoutPlaygroundView: View {
    ...
    private var installmentsSection: some View {
        Section("Installments") {
            Picker("Min", selection: self.$config.minInstallments) {
                ForEach(1 ... self.config.maxInstallments, id: \.self) { value in
                    Text("\(value)")
                        .tag(value)
                        .accessibilityIdentifier("playground.minInstallments.\(value)")
                }
            }
            .accessibilityIdentifier("playground.minInstallments")
            Picker("Max", selection: self.$config.maxInstallments) {
                ForEach(self.config.minInstallments ... 180, id: \.self) { value in
                    Text("\(value)")
                        .tag(value)
                        .accessibilityIdentifier("playground.maxInstallments.\(value)")
                }
            }
