...
class CardFormViewModel: ObservableObject {
    ...
    func handleBinChange(_ bin: String) async {
        ...
        await self.searchInstallment(bin: bin)
    ...
    func createPaymentToken(
        ...
        if let selectedDocumentType {
            tokenData = try await self.coreMethods.createToken(
                cardNumber: cardNumber,
                expirationDate: expirationDate,
                securityCode: securityCode,
                documentType: selectedDocumentType,
                documentNumber: self.documentText,
    ...
    private func searchPaymentMethod(bin: String) async {
        do {
            ...
            _ = try await self.coreMethods.issuers(bin: bin, paymentMethodID: paymentMethod.id)
            ...
            if let maxSecurityCode = paymentMethod.card?.securityCode.length {
                self.maxLengthSecurityCode = maxSecurityCode
            }
