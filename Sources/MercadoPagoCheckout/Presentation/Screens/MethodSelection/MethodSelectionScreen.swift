...
struct MethodSelectionScreen: View {
    ...
    private let onOptionSelected: @MainActor (MethodSelectionOutput.Option) async -> Void
    private let onBack: () -> Void

    init(
        viewModel: MethodSelectionViewModel,
        onOptionSelected: @escaping @MainActor (MethodSelectionOutput.Option) async -> Void = { _ in },
    ...
    var body: some View {
        ...
        .allowsHitTesting(!self.viewModel.isConfirming)
                ...
                buttonData: .init(
                    ...
                    onClick: {
                        if let option = self.viewModel.confirmSelection() {
                            await self.submitSelection(option)
                        }
                    }
                )
            )
            .isLoading(self.viewModel.isConfirming)
            .disabled(!self.viewModel.isCtaEnabled && !self.viewModel.isConfirming)
        } else {
            MPFooter(
                title: self.viewModel.output.footer.totalLabel,
                amount: amount
    ...
    private func submitSelection(_ option: MethodSelectionOutput.Option) async {
        defer { self.viewModel.finishSelection() }
        ...
        Binding(
            ...
            set: { isSelected in
                ...
                if let selected = self.viewModel.selectOption(option.id) {
                    Task { await self.submitSelection(selected) }
                }
            }
        )
    }
