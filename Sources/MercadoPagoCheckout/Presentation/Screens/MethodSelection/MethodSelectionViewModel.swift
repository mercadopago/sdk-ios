...
final class MethodSelectionViewModel: ObservableObject {
    ...
    var isCtaEnabled: Bool { self.selectedOptionId != nil && !self.isConfirming }
    ...
    func selectOption(_ optionId: String) -> MethodSelectionOutput.Option? {
        guard !self.isConfirming,
              let option = self.output.options.first(where: { $0.id == optionId })
        else { return nil }
    ...
    func confirmSelection() -> MethodSelectionOutput.Option? {
        guard !self.isConfirming,
              ...
              let option = self.output.options.first(where: { $0.id == selectedId })
        else { return nil }
