...
package enum NativeErrorClassifier {
    ...
    private static func outputCode(for input: NativeErrorInput) -> NativeErrorCode {
        if input.type == .userCancellation {
            return .userCancelled
        }
        if input.type == .requestCancellation || input.code == .cancelled {
            return .requestCancelled
        }
        if isConfiguration(input) {
            return .sdkConfigurationInvalid
        }
        if input.type == .validation {
            return .inputValidationFailed
        }
        if input.responseState != nil || input.code == .emptyBody {
            return .responseContractInvalid
        }
        if isConnection(input.code) {
            return .connectionUnavailable
        }
        if input.code == .timeout || input.httpStatus == 408 || input.httpStatus == 504 {
            return .requestTimeout
        }
        if input.type == .request || input.type == .service {
    ...
    private static func diagnosticCode(
        ...
        if outputCode == .userCancelled || outputCode == .requestCancelled {
            return .cancelled
        }
        if input.code == .invalidURL {
            return .invalidURL
        }
        if input.code == .httpUnauthorized || input.httpStatus == 401 {
            return .httpUnauthorized
        }
        if input.code == .httpForbidden || input.httpStatus == 403 {
            return .httpForbidden
        }
        if outputCode == .inputValidationFailed {
            return .validation
        }
        if let responseState = input.responseState {
            return responseState == .emptyBody ? .emptyBody : .decodeFailure
        }
        if input.code == .emptyBody {
            return .emptyBody
        }
        switch input.code {
