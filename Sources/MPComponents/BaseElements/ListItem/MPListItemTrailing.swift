...
/// Passive content uses `MPListItemTrailingStyle`; when an action is present, `MPListItem`
/// renders it as an independently tappable button.
package struct MPListItemTrailing {
    ...
    /// instead of a passive label. `nil` keeps the row's own tap gesture (row selection) as the
