...
final class MPTouchDelegatingView: UIView {
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        return backView?.hitTest(converted, with: event)
    }
}
...
final class MPPopoverController: UIPopoverPresentationController, UIPopoverPresentationControllerDelegate {
    ...
    override func adaptivePresentationStyle(for traitCollection: UITraitCollection) -> UIModalPresentationStyle {
        delegate?.adaptivePresentationStyle?(for: self, traitCollection: traitCollection) ?? adaptivePresentationStyle
    }
    ...
    override func containerViewDidLayoutSubviews() {
        ...
        if let containerView {
            hideCutoutShadowView(in: containerView)
        }
    }
    ...
    override func presentationTransitionWillBegin() {
        ...
        overlayView.backView = presentingViewController.view
        containerView.insertSubview(overlayView, at: 0)

        // Safety: ensure the presented view is in the container (mirrors Andes's addSubview guard)
        let rootView = presentedViewController.view!
        if rootView.superview == nil {
            containerView.addSubview(rootView)
        }
        rootView.frame = frameOfPresentedViewInContainerView

        hideCutoutShadowView(in: containerView)
        containerView.window.map { hideCutoutShadowView(in: $0) }
    }
    ...
    private func hideCutoutShadowView(in view: UIView) {
        ...
        view.subviews.forEach { hideCutoutShadowView(in: $0) }
    }
}
...
struct MPPopoverPresenter<Content: View>: UIViewControllerRepresentable {
    ...
    func updateUIViewController(_ backgroundVC: UIViewController, context: Context) {
        if isPresented.wrappedValue {
            ...
            // and UIKit silently ignores present() calls made during layout. Mirrors Andes pattern.
