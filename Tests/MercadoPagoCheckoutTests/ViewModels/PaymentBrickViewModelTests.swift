...
final class PaymentBrickViewModelTests: XCTestCase {
    ...
    func test_cardTransaction_withToken_mapsSavedCardPaymentData() throws {
        ...
        XCTAssertEqual(result.transactionAmount, .zero)
    }

    func test_cardTransaction_whenInstallmentsAreAvailable_setsTransactionAmountFromFirstQuota() throws {
        let sut = self.makePaymentSUT()
        let item = self.makeSavedCardItem(
            withSecurityCode: true,
            installments: self.makeInstallmentsData()
        )

        let result = try XCTUnwrap(sut.cardTransaction(from: item, token: "token-123"))

        XCTAssertEqual(result.transactionAmount, 100)
    }

    // MARK: - cardTransaction(from: CardFormSubmitResult, amount:)

    func test_cardTransaction_fromCardFormSubmitResult_mapsTokenizedPaymentData() {
        let sut = self.makePaymentSUT()
        let result = self.makeCardFormSubmitResult(token: "new-card-token-456")

        let transaction = sut.cardTransaction(from: result, amount: 250)

        // AC: the payload is built from the tokenized card_token, not raw card fields —
        // CardFormSubmitResult carries no PAN/CVV/holder/expiry to begin with.
        XCTAssertEqual(transaction.token, "new-card-token-456")
        XCTAssertEqual(transaction.paymentMethodId, "visa")
        XCTAssertEqual(transaction.paymentTypeId, "credit_card")
        XCTAssertEqual(transaction.issuerId, "1")
        XCTAssertEqual(transaction.orderId, "ORD01")
    }

    func test_cardTransaction_fromCardFormSubmitResult_withoutInstallments_usesGivenAmount() {
        let sut = self.makePaymentSUT()
        let result = self.makeCardFormSubmitResult(installmentsData: nil)

        let transaction = sut.cardTransaction(from: result, amount: 250)

        XCTAssertEqual(transaction.transactionAmount, 250)
    }

    func test_cardTransaction_fromCardFormSubmitResult_withInstallments_stillUsesGivenAmount() {
        let sut = self.makePaymentSUT()
        let result = self.makeCardFormSubmitResult(installmentsData: self.makeMPInstallmentsData())
    ...
    // MARK: - Current payment mapping

    func test_processOrder_WhenProcessedCard_ShouldExposeOrderStatusDetail() async throws {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData(statusDetail: "accredited")))

        let result = try await sut.processOrder(params: self.makeOrderParams())

        XCTAssertEqual(result.orderStatusDetail, "accredited")
    }

    func test_processOrder_WhenTicketActionRequired_ShouldExposeCurrentPayment() async throws {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData(
        ...
        XCTAssertEqual(result.orderStatus, "action_required")
        XCTAssertEqual(result.orderStatusDetail, "waiting_payment")
        XCTAssertEqual(result.paymentMethodId, "rapipago")
        XCTAssertEqual(result.paymentTypeId, "ticket")
        XCTAssertEqual(result.transactionAmount, Decimal(string: "250.50"))
    }

    func test_processOrder_WhenPaymentHasOwnAmount_ShouldPreferItOverOrderTotal() async throws {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData(totalAmount: "100.00", paymentAmount: "42.75")))

        let result = try await sut.processOrder(params: self.makeOrderParams())

        XCTAssertEqual(result.transactionAmount, Decimal(string: "42.75"))
    }

    func test_processOrder_WhenPaymentAmountMissing_ShouldUseOrderTotal() async throws {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData(totalAmount: "199.99", paymentAmount: nil)))

        let result = try await sut.processOrder(params: self.makeOrderParams())

        XCTAssertEqual(result.transactionAmount, Decimal(string: "199.99"))
    }

    func test_processOrder_WhenAmountIsInvalid_ShouldThrowServiceError() async {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData(totalAmount: "not-a-number")))

        do {
            _ = try await sut.processOrder(params: self.makeOrderParams())
            XCTFail("Expected throw")
        } catch let err as MercadoPagoCheckoutError {
            XCTAssertEqual(err.code, .serviceError)
            XCTAssertEqual(err.locationDescription, "orderProcess")
        } catch {
    ...
    // MARK: - processOrderResult

    func test_processOrderResult_WhenProcessed_ShouldReturnSuccess() async {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData()))

        guard case let .success(payment) = await sut.processOrderResult(params: self.makeOrderParams()) else {
            return XCTFail("Expected success")
        }
        XCTAssertEqual(payment.orderStatus, "approved")
    }

    func test_processOrderResult_WhenRepositoryFails_ShouldReturnError() async {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        let error = MercadoPagoCheckoutError(code: .serviceError, localizedDescription: "fail", location: .orderProcess)
        await repository.setResult(.failure(error))

        guard case let .error(returned) = await sut.processOrderResult(params: self.makeOrderParams()) else {
            return XCTFail("Expected error")
        }
        XCTAssertEqual(returned.locationDescription, "orderProcess")
    }

    func test_processOrderData_WhenConfirmedOnce_ShouldCallTheRepositoryOnce() async throws {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData()))

        _ = try await sut.processOrderData(params: self.makeOrderParams())

        let count = await repository.callCount
        XCTAssertEqual(count, 1)
    }

    func test_processOrderData_ShouldSendTraceabilityBuiltByTheUseCase() async throws {
        let repository = MockOrderTransactionRepository()
        let sut = self.makePaymentSUT(orderRepository: repository)
        await repository.setResult(.success(self.makeProcessData()))
        ...
        let sentParams = await repository.lastParams
        XCTAssertEqual(sentParams?.integrationData?.feature, .payment)
    }
    ...
    private func makePaymentSUT(
        ...
        if let repo = orderRepository {
            return PaymentBrickViewModel(
                configuration: configuration,
                orderTransactionUseCase: OrderTransactionUseCase(repository: repo)
            )
        }
    ...
    private func makeCardFormSubmitResult(
        token: String = "new-card-token",
        installmentsData: MPInstallmentsData? = nil
    ) -> CardFormSubmitResult {
        CardFormSubmitResult(
            token: token,
    ...
    private func makeMPInstallmentsData() -> MPInstallmentsData {
        MPInstallmentsData(
            installment: self.makeInstallmentsData(),
            cardDisplayInfo: CardDisplayInfo(
                issuerName: "Visa Crédito",
                paymentTypeId: "credit_card",
                lastFourDigits: "1111"
            )
        )
    ...
    private func makeOrderParams() -> OrderTransactionParams {
        OrderTransactionParams(
            amount: 100,
            paymentMethodType: .creditCard(paymentMethodId: "visa", paymentTypeId: "credit_card", token: "tok123", installments: 1)
        )
    }

    /// `paymentAmount: nil` reproduces a processed payment without its own amount, so the
    /// mapping falls back to the order total.
