...
final class CardFormBrickViewModelReviewConfirmTests: XCTestCase {
    ...
    func test_reviewConfirmInput_withDebitCard_omitsInstallments() throws {
        ...
        let input = try XCTUnwrap(
            sut.reviewConfirmInput(
                cardTransaction: self.makeCardTransaction(paymentTypeId: "debit_card", installment: 1),
                inputCardData: InputCardData(bin: "41111111", lastFourDigits: "1234"),
                installmentAmount: 100
            )
        )

        // Assert: debit is its own case and carries no installments at all
        guard case let .debitCard(paymentMethodId, paymentTypeId, token) = input.paymentParams.paymentMethodType else {
            return XCTFail("Expected .debitCard case")
        }
        XCTAssertEqual(paymentTypeId, "debit_card")
        XCTAssertFalse(paymentMethodId.isEmpty)
        XCTAssertFalse(token.isEmpty)
        XCTAssertNil(input.cardDetails.installments)
    }
