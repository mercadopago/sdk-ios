...
final class FetchReviewConfirmUseCaseTests: XCTestCase {
    ...
    private func makeCardParams() -> OrderTransactionParams {
    ...
    private func makeTicketParams() -> OrderTransactionParams {
        OrderTransactionParams(amount: 100, paymentMethodType: .ticket(paymentMethodId: "rapipago", paymentTypeId: "ticket"))
    }
    ...
    func test_execute_withDebitCard_omitsInstallmentFields() async throws {
        ...
        // Assert
        let request = await repository.lastRequest
        XCTAssertEqual(request?.paymentMethodType, "debit_card")
        XCTAssertEqual(request?.paymentMethodId, "debvisa")
