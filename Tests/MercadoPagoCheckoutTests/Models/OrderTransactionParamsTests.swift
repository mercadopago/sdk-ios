...
final class OrderTransactionParamsTests: XCTestCase {
    ...
    private func makeTicketParams(
        ...
        paymentTypeId: String = "ticket"
    ) -> OrderTransactionParams {
        OrderTransactionParams(
            amount: amount,
            paymentMethodType: .ticket(paymentMethodId: paymentMethodId, paymentTypeId: paymentTypeId)
        )
    }
    ...
    func testEncoding_ticket_paymentMethodType_defaultsToTicketValue() throws {
        let json = try encodeToJSON(makeTicketParams())
        XCTAssertEqual(json["payment_method_type"] as? String, "ticket")
    }

    func testEncoding_ticket_paymentMethodType_usesGivenValue_notHardcoded() throws {
        // Regression: payment_method_type must come from `sections[ticket_item].type` — never a
        // literal "ticket" — so any future offline payment type category is forwarded as-is.
        let json = try encodeToJSON(makeTicketParams(paymentTypeId: "atm"))
        XCTAssertEqual(json["payment_method_type"] as? String, "atm")
    }
