...
final class PaymentBrickNavigationTests: XCTestCase {
    ...
    func test_installmentsBackTransition_whenPreviousRouteIsCardForm_returnsCardFormWithoutRecreatingScreen() {
        let transition = SUT.installmentsBackTransition(from: .cardForm)

        XCTAssertEqual(transition.destination, .cardForm)
        XCTAssertFalse(transition.shouldRecreateSecurityCode)
    }
}
