...
final class NativeErrorClassifierTests: XCTestCase {
    ...
    func testCanonicalClassificationVectors() throws {
        ...
        for vector in fixture.vectors {
            ...
            let input = try NativeErrorInput(
                type: XCTUnwrap(NativeErrorInputType(rawValue: vector.input.type), vector.id),
    ...
    func testAPIClientErrorUsesOneSanitizedInfrastructureMapping() {
        let cases: [(APIClientError, NativeErrorInput)] = [
            (.invalidURL, .init(type: .request, code: .invalidURL)),
            (.urlRequestIsEmpty, .init(type: .request, code: .invalidURL)),
            (.invalidResponse(Data()), .init(type: .request, responseState: .emptyBody)),
            (.invalidResponse(Data("non-empty".utf8)), .init(type: .unknown, code: .exception)),
            (.decodingFailed(TestError()), .init(type: .request, responseState: .decodeFailure)),
            (.networkError(URLError(.timedOut)), .init(type: .request, code: .timeout)),
            (.requestFailed(URLError(.notConnectedToInternet)), .init(type: .request, code: .offline)),
            (.statusCode(503), .init(type: .service, httpStatus: 503)),
            (.notExpectedHttpResponseCode(code: 504), .init(type: .service, httpStatus: 504)),
            (.apiError(.init(code: "bad_request", message: "ignored")), .init(type: .service)),
        ]

        for (error, expected) in cases {
            XCTAssertEqual(error.nativeErrorInput, expected)
        }
    }
