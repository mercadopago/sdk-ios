...
final class NativeErrorReportTests: XCTestCase {
    func testReportEncodesOnlyAllowlistedFields() throws {
        let pending = try PendingNativeError(
            eventID: XCTUnwrap(UUID(uuidString: "3f6fd694-4ba8-4f45-ae7c-871c4698aace")),
    ...
    func testInvalidOptionalValuesAreOmittedConstructively() {
    ...
    func testOccurredAtPreservesSubsecondPrecision() throws {
        let pending = try PendingNativeError(
            eventID: XCTUnwrap(UUID(uuidString: "3f6fd694-4ba8-4f45-ae7c-871c4698aace")),
    ...
    func testEnvironmentSnapshotsDoNotChangeRetroactively() async {
        let environment = NativeErrorEnvironment(osVersionProvider: { "18.6" })
        await environment.configure(.init(sdkVersion: "1.0.0", siteID: "MLB"))
        let first = await environment.snapshot()
        await environment.configure(.init(sdkVersion: "2.0.0", siteID: "MCO"))
        let second = await environment.snapshot()

        XCTAssertEqual(first, .init(sdkVersion: "1.0.0", siteID: "MLB", osVersion: "18.6"))
        XCTAssertEqual(second, .init(sdkVersion: "2.0.0", siteID: "MCO", osVersion: "18.6"))
    }
}
